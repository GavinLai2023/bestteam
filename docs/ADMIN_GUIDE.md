...
| `migrate-db --to <url> [--fix-orphans] [--batch-size N]` | Copy this deployment's database into an **empty** one — the Postgres cutover's first step. **Stop the application first**: the copy is a point-in-time snapshot, and a source still being written to fails verification rather than copying silently. `--batch-size` is the rows per INSERT (default 1000). Pre-flight refuses a source behind head, a non-empty target, the same URL, and dangling foreign keys unless `--fix-orphans` (nullable → NULL, NOT NULL → row skipped, both reported). Never writes to the source. The memory store and the files under `ui/backend/data` are not copied. |
