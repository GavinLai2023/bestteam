...
|---|---|---|---|---|---|---|
| CR-001 | Public registration plus unrestricted KB paths enables server-file replacement | Confirmed; boundary guards + resolved-root containment | Agree | cache_path is now confined to an app-owned `_kb_cache/` subdir at every backend boundary AND at load (persisted/legacy + inline records); model-generated stored specs are contained. Registration gating + non-root container + generation-time transient write remain follow-up units | Accept containment; keep registration/container follow-ups open | Fixed (containment) - awaiting independent verification |
...
| CR-008 | KB upload replacement is non-atomic and has destructive rollback | Confirmed | Agree | Data-integrity resolved (atomic promote, commit-before-discard, restore-failure preserves last copy). A concurrent-reader gap remains (live_dir briefly absent during the cross-platform dir swap) -- needs a versioned-dir + atomic-pointer layout | Accept integrity fix; keep swap-window follow-up open | Partially resolved |
| CR-009 | Interview upload and ffmpeg processing are unbounded | Confirmed | Agree | ffmpeg timeouts + capped read; body-size **middleware** now rejects oversize by Content-Length before Starlette parses/spools (FastAPI reads request.form() before dependencies, so the earlier dependency was ineffective). Proxy client_max_body_size remains the hard bound | Accept fix | Fixed - awaiting independent verification |
