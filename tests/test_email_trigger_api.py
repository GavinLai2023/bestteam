...
def _outcome_row(db, org_id, uid, status):
    from ui.backend.db.models import DraftOutcome

    db.add(DraftOutcome(org_id=org_id, run_id=_seed_run(db, org_id),
                        source_key=f"mailbox:7:uidvalidity:3:uid:{uid}",
                        status=status))
    db.commit()


def _seed_run(db, org_id):
    run_id = f"run-{org_id}-{db.query(Run).count()}"
    db.add(Run(id=run_id, pipeline="triage", input="in", output="out",
               status="completed", org_id=org_id, username="email-trigger"))
    db.commit()
    return run_id


def test_draft_outcomes_counts_are_org_scoped(client):
    org_id = _seed_team(_EMAIL_TEAM_CONFIG)
    with open_test_db() as db:
        other = get_or_create_org(db, "other")
        other_id = other.id
        _outcome_row(db, org_id, "1", "sent")
        _outcome_row(db, org_id, "2", "sent")
        _outcome_row(db, org_id, "3", "handled")
        _outcome_row(db, org_id, "4", "pending")
        _outcome_row(db, org_id, "5", "unknown")
        _outcome_row(db, other_id, "6", "sent")

    body = client.get("/api/org/email-trigger/draft-outcomes").json()
    assert body == {"sent": 2, "handled": 1, "pending": 1, "window_days": 30}


def test_draft_outcomes_requires_auth(client):
    resp = client.get("/api/org/email-trigger/draft-outcomes",
