...
def _deployed_team(db, org_id, name="team1"):
    record = PipelineRecord(
        ...
        config={"name": name, "agents": [], "teams": [], "pipeline": {"steps": []}},
        status="deployed",
    )
    db.add(record)
    db.commit()
...
def test_get_share_link_scoped_to_org(db):
    ...
    link = create_share_link(db, pipeline_id=team.id, org_id=org_a.id, created_by=user.id)
...
def test_list_share_links_newest_first(db):
    ...
    user = create_user(db, "owner", "pw", org_id=org.id)
    team = _deployed_team(db, org.id)
    first = create_share_link(db, pipeline_id=team.id, org_id=org.id, created_by=user.id)
    second = create_share_link(db, pipeline_id=team.id, org_id=org.id, created_by=user.id)
...
def test_count_active_share_links(db):
    ...
    user = create_user(db, "owner", "pw", org_id=org.id)
    team = _deployed_team(db, org.id)
    a = create_share_link(db, pipeline_id=team.id, org_id=org.id, created_by=user.id)
    create_share_link(db, pipeline_id=team.id, org_id=org.id, created_by=user.id)
