...
def _share_session(db):
    ...
    team = PipelineRecord(
        ...
        config={"name": "t", "agents": [], "teams": [], "pipeline": {"steps": []}},
        status="deployed",
    )
    db.add(team)
    db.commit()
...
def test_records_the_reply_for_a_share_run(db):
    ...
    run_row = Run(
        id="run-1", pipeline="t", input="hi", org_id=1,
...
def test_falls_back_to_a_friendly_message_when_output_is_none(db):
    ...
    append_message(db, session.id, turn_number=1, role="user", content="hi")
    run_row = Run(
        id="run-2", pipeline="t", input="hi", org_id=1,
        trigger_context={"share_link_id": 1, "share_session_id": session.id, "turn_number": 1},
    )
    db.add(run_row)
    db.commit()
...
def test_calling_twice_for_the_same_turn_is_a_no_op(db):
    ...
    append_message(db, session.id, turn_number=1, role="user", content="hi")
    run_row = Run(
        id="run-6", pipeline="t", input="hi", org_id=1,
        trigger_context={"share_link_id": 1, "share_session_id": session.id, "turn_number": 1},
    )
    db.add(run_row)
    db.commit()
...
def test_no_op_for_a_run_without_trigger_context(db):
    ...
    run_row = Run(id="run-3", pipeline="t", input="hi", org_id=1, trigger_context=None)
...
def test_no_op_for_a_run_with_unrelated_trigger_context(db):
    ...
    run_row = Run(
        id="run-4", pipeline="t", input="hi", org_id=1,
...
def test_run_in_background_records_share_reply_on_success(db, monkeypatch):
    # `_build_pipeline` (core/loader.py) requires `source`/`extra_tools`
    # keyword-only args that the plan brief's snippet omitted (signature
    # drift since the brief was written) -- build the same Pipeline directly
    # via the SDK dataclasses instead, following the established pattern in
    # tests/test_memory_backend.py::_pipeline().
    ...
    pipeline = Pipeline(name="t", steps=[Team(name="tm", agents=[agent], mode=CollaborationMode.SEQUENTIAL)])

    db.add(Run(
        id="run-5", pipeline="t", input="hi", org_id=1,
        trigger_context={"share_link_id": 1, "share_session_id": session.id, "turn_number": 1},
    ))
    db.commit()

    run_in_background("run-5", pipeline, "hi", engine=db.get_bind())
