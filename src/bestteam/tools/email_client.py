...
class _ImapBackend:
    ...
    def sent_with_source_keys(self, source_keys) -> set:
        """The subset of `source_keys` that has a message in the Sent folder
        (matched on the `X-BestTeam-Source-Key` header).

        Draft outcome tracking's strongest send evidence: the client kept the
        platform's own MIME, custom header included. Read-only.
        """
        return self._search_sent_header(_SOURCE_KEY_HEADER, source_keys)

    def sent_replies_to(self, message_ids) -> set:
        """The subset of `message_ids` some Sent-folder message is a reply to
        (matched on the `In-Reply-To` header).

        Fallback send evidence for clients that rebuild the MIME on send and
        drop the custom header -- `In-Reply-To` is regenerated by every
        reasonable mail client. Read-only.
        """
        return self._search_sent_header("In-Reply-To", message_ids)

    def _search_sent_header(self, field: str, values) -> set:
        ...
        try:
            ...
            found = set()
            for value in wanted:
                typ, data = conn.uid("search", None, "HEADER", field, f'"{value}"')
                if typ == "OK" and data and data[0]:
                    found.add(value)
            return found
        finally:
    ...
    def message_ids_for_uids(self, uids) -> dict:
        """RFC Message-IDs of the given INBOX UIDs, `{uid: message_id}`.

        Feeds `sent_replies_to`: outcome tracking only knows a draft by the
        UID of the customer email it replies to. A UID whose message is gone
        (or has no Message-ID) is simply absent from the result. Read-only.
        """
        wanted = [str(u) for u in uids if str(u)]
        if not wanted:
            return {}
        conn = self._connect()
        try:
            ...
            if typ != "OK":
                raise OSError(f"IMAP SELECT of 'INBOX' failed: {typ}")
            out = {}
            for uid in wanted:
                typ, data = conn.uid(
                    "fetch", uid.encode(), "(BODY.PEEK[HEADER.FIELDS (MESSAGE-ID)])"
                )
                raw = _imap_fetch_bytes(typ, data)
                if not raw:
                    continue
                message_id = message_from_bytes(raw).get("Message-ID")
                if message_id:
                    out[uid] = str(message_id).strip()
            return out
        finally:
            _imap_logout(conn)

    def _sent_folder(self, conn) -> str:
        """Resolve the Sent folder via the `\\Sent` special-use flag (mirrors
        `_drafts_folder`; covers Gmail's `[Gmail]/Sent Mail` and Microsoft's
        `Sent Items`), defaulting to "Sent"."""
        typ, data = conn.list()
        if typ == "OK":
            for line in data or []:
